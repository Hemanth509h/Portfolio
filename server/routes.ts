...
import { insertContactSchema, insertPortfolioDataSchema } from "@shared/schema";
import { z } from "zod";
...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  if (!process.env.ADMIN_CODE) {
    console.log('⚠️  Using default admin code "admin123" for development');
    console.log('💡 Set ADMIN_CODE environment variable for production');
  }

  // Admin code middleware
  function requireAdminCode(req: any, res: any, next: any) {
  ...
  app.post("/api/admin/verify", (req, res) => {
    try {
      const { code } = req.body;
      if (code === ADMIN_CODE) {
        res.json({ success: true, message: "Admin code verified" });
      } else {
  ...
  app.put("/api/admin/portfolio", requireAdminCode, async (req, res) => {
    try {
